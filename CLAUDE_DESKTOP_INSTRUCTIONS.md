...
3. **Fact-check with formatted transcript (if available)**:
   - **Try to load**: Call `get_formatted_transcript(project_name)` to check if it's available
   - **If available**: Use the AP Style formatted version to verify:
     - Speaker names and titles
     - Direct quotes (exact wording)
     - Facts mentioned in the video
     - Proper nouns (places, organizations, etc.)
   - **If not available**: Proceed using the brainstorming document and raw transcript
     - The brainstorming document already contains key quotes and information from the transcript
     - You can still provide thorough copy revision based on this content
     - Note in your revision that formatted transcript verification can be done later if needed
   - **IMPORTANT**: When formatted transcript IS available, it is the authoritative source for accuracy
...
2. **Fact-check with formatted transcript (if available)**:
   - **Try to load**: Call `get_formatted_transcript(project_name)` to check if it's available
   - **If available**: Use it for thorough fact-checking:
   ...
   - **If not available**: Use the raw transcript and brainstorming document for verification
     - Cross-reference against the transcript content provided in project load
     - Note which details should be verified when formatted transcript becomes available
...
**When to use**: When you need enhanced accuracy verification against the source material

**Availability**: Not all projects have formatted transcripts yet. They're generated by the formatter agent in Claude Code after brainstorming is complete.

**How to check availability**:
```
get_formatted_transcript(project_name)
```
- If available: Returns the AP Style formatted transcript
- If not available: Returns a message indicating it hasn't been generated yet
...
**If not available**: You can still complete copy revision work using:
- The brainstorming document (which includes key quotes and facts)
- The raw transcript content (provided in project load)
- Note in your revision which details should be re-verified when formatted transcript becomes available
...
**Best practice**:
- When formatted transcript is available and you need to verify specific details, use it as the authoritative source
- When formatted transcript is NOT available, proceed with your copy revision work using the brainstorming and raw transcript
- **Don't wait or get stuck** - the brainstorming document contains sufficient information to create quality copy revisions
